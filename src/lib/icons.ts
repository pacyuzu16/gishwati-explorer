/**
 * Lightweight inline icon set (Lucide-style, MIT). Returns SVG strings so we
 * avoid an icon dependency and any emoji. Stroke uses currentColor.
 */
...
export function icon(name: keyof typeof PATHS | string, cls = "h-5 w-5"): string {
  const p = PATHS[name] || "";
